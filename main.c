...
static void focus_toplevel(struct turtile_toplevel *toplevel, struct wlr_surface *surface) {
...
static bool handle_keybinding(struct turtile_server *server, xkb_keysym_t sym) {
	/*
...
static void server_new_keyboard(struct turtile_server *server,
...
static void server_new_pointer(struct turtile_server *server,
		struct wlr_input_device *device) {
...
static struct turtile_toplevel *desktop_toplevel_at(
...
static void reset_cursor_mode(struct turtile_server *server) {
	/* Reset the cursor mode to passthrough. */
	server->cursor_mode = TURTILE_CURSOR_PASSTHROUGH;
	server->grabbed_toplevel = NULL;
}

static void process_cursor_move(struct turtile_server *server, uint32_t time) {
...
static void process_cursor_resize(struct turtile_server *server, uint32_t time) {
	/*
...
static void process_cursor_motion(struct turtile_server *server, uint32_t time) {
...
static void output_frame(struct wl_listener *listener, void *data) {
	...
	struct turtile_output *output = wl_container_of(listener, output, frame);
...
static void output_request_state(struct wl_listener *listener, void *data) {
	...
	struct turtile_output *output = wl_container_of(listener, output, request_state);
	const struct wlr_output_event_request_state *event = data;
	wlr_output_commit_state(output->wlr_output, event->state);
}

static void output_destroy(struct wl_listener *listener, void *data) {
	struct turtile_output *output = wl_container_of(listener, output, destroy);
...
static void server_new_output(struct wl_listener *listener, void *data) {
	...
	struct turtile_output *output = calloc(1, sizeof(*output));
...
static void begin_interactive(struct turtile_toplevel *toplevel,
...
int main(int argc, char *argv[]) {
	...
	struct turtile_server server = {0};
